...
\f0\fs24 \cf0 import type \{ CSSProperties, ReactNode \} from 'react';\
...
export function PdfPreview(\{ client, season \}: Props) \{\
  ...
  ].map((value) => value.toUpperCase()).join(' \'b7 ');\
...
\
      <PdfPage page=\{3\}>\
        ...
        <div className="pdf-styling-advice">\
          <section><h3>Metals</h3><p className="pdf-metals">\{styling.metals.join(' \'b7 ')\}</p></section>\
          <section><h3>Your Contrast</h3><p>\{styling.contrastStrategy\}</p></section>\
          <section><h3>Colours to Avoid</h3><p>\{styling.avoid.join(' \'b7 ')\}</p></section>\
